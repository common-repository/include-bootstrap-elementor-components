        ...
        <?php 
            if( $the_query->have_posts() ) {
                ...
                while( $the_query->have_posts() ) {
                    $the_query->the_post();
                    ?>
                    <div class="col-md-3 col-xs-12 ebc-blog-margin">
                        
                        <!--Flip Rotate Top-->
                        <figure class="ebc-modern-hover-22-rotate-top" style="transition: all <?php echo $settings['transtion-time']; ?>s ease; transition-timing-function: <?php echo $settings['transtion-type'];?>; ">
                            <a id="post-<?php the_ID(); ?>" <?php post_class('modern_hover_image'); ?> href="<?php the_permalink(); ?>" style="transition: all <?php echo $settings['transtion-time']; ?>s ease; transition-timing-function: <?php echo $settings['transtion-type'];?>; ">
                                <?php the_post_thumbnail(); ?>
                            </a>
                            <figcaption style="transition: <?php echo $settings['transtion-time']; ?>s ease; transition-timing-function: <?php echo $settings['transtion-type'];?>; text-align: <?php echo $settings['blog_content_alingment'];?>;">	
                                <div class="ebc-post-content" >
